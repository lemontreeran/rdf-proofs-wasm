...
describe('Revocable Proofs', () => {
  test('blind sign and verify', async () => {
    await initializeWasm();

    const userId = new Uint8Array(Buffer.from('USER-ID'));

    const proof = sign(doc1, proofWithoutProofvalue1, keyGraph, userId);
    expect(proof).toBeDefined();

    const verified = blindVerify(userId, doc1, proof, keyGraph);
    expect(verified.verified).toBeTruthy();
  });

  test('get encrypted user id', async () => {
    await initializeWasm();

    const userId = new Uint8Array(Buffer.from('USER-ID'));
    const encryptedUserId = getEncryptedUid(userId);
    console.log(encryptedUserId);
    expect(encryptedUserId).toBeDefined();
  });

  test('deriveProof with revocable secret', async () => {
    await initializeWasm();
    ...
    const req: DeriveProofRequest = {
      ...
      challenge,
      secret,
      openerPubKey: keyPair.publicKey,
    };
